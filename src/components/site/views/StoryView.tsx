...
export function StoryView() {
  ...
  const categoryIcon = (cat: string) => {
    if (cat === 'award') return <Award className="text-[#C9A961]" />;
    if (cat === 'meeting') return <Briefcase className="text-[#0A1F44]" />;
    if (cat === 'celebration') return <Users className="text-[#A68A3F]" />;
    return <Calendar className="text-[#C9A961]" />;
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#F9FAFB] to-white">
      ...
      <section className="max-w-6xl mx-auto px-6 py-16">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-3xl font-serif text-[#0A1F44]">Featured Moments</h2>
          {events.length > 1 && (
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-slate-500">
            ...
            <p>Story events will appear here soon.</p>
          </div>
        ) : (
          <AnimatePresence mode="wait">
            <motion.div
              ...
              onClick={() => openModal(events[activeIndex])}
            >
              {parseImages(events[activeIndex].images)[0] ? (
                <img src={parseImages(events[activeIndex].images)[0]} alt={events[activeIndex].title} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
              ) : (
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-[#C9A961] text-sm uppercase tracking-wider">{events[activeIndex].category}</span>
          ...
          <div className="relative">
            ...
            {events.map((e, i) => (
              <motion.div
                ...
                onClick={() => { setActiveIndex(i); openModal(e); }}
              >
          ...
          >
            ...
            >
              <div className="relative h-64 bg-[#0A1F44]">
                {parseImages(modalEvent.images)[0] && (
                  <img src={parseImages(modalEvent.images)[0]} alt={modalEvent.title} className="w-full h-full object-cover" />
                )}
              ...
              <div className="p-8">
                ...
                {parseImages(modalEvent.images).length > 1 && (
                  <div className="grid grid-cols-3 gap-3 mt-6">
                    {parseImages(modalEvent.images).slice(1).map((img, i) => (
