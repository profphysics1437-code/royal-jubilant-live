#!/bin/bash
# Apply all UI refinements from July 8-9 work period
set -e
cd /home/z/my-project

echo "=== 1. Hero search bar (premium 2-col) ==="
# Rebuild Hero with Buy/Rent toggle + Community + Advanced Search
cat > src/components/site/Hero.tsx << 'HERO_EOF'
"use client";
...
echo "=== 2. Media upload endpoint ==="
mkdir -p src/app/api/admin/upload src/app/api/agent/upload public/uploads
...
EOF
cat > src/app/api/agent/upload/route.ts << 'EOF'
export { POST, GET } from '@/app/api/admin/upload/route';
EOF
echo "  ✓ Upload endpoints created"

echo "=== 3. useApi stale data fix ==="
cat > src/lib/useApi.ts << 'EOF'
"use client";
